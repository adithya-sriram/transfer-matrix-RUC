...
MatrixXd generic_state(vector<int> sites, int L)
{
    MatrixXd up(2,1); 
    MatrixXd dn(2,1); 
    up.coeffRef(0,0) = 1;
...
MatrixXd transfer_matrix(int L)
{

    MatrixXd T(int(pow(2,L)), int(pow(2,L)));
...
MatrixXd final_state(int site, int L) {
...
MatrixXd all_states_with_width(int L, int w) {
    ...
    MatrixXd proj = kroneckerProduct(state_vectors[0], state_vectors[0].transpose());
    for (int i = 1; i < state_vectors.size(); i++) {
        MatrixXd s = state_vectors[i];
...
map<int, vector<double>> correlation_function_cutoff(int t, int L) {
    vector<int> sites;
    sites.push_back(0);
    MatrixXd initial_bc = generic_state(sites, t);
    int d = 4;
    MatrixXd M = transfer_matrix(t);
    MatrixXd Pn;
    ...
    for (int w = 0; w < t; w++) {
        ...
        for (int k = 0; k < t; k++) {
            MatrixXd final_bc = final_state(k, t);
            MatrixXd correlator = initial_bc.transpose() * M_t * final_bc;
...
{
    int L = 8;
    correlation_function_cutoff(12,12);
}